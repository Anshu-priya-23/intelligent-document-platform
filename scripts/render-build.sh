...
# Install Debian packages into the application directory without root or Docker.
ocr_root="$PWD/.local/ocr"
mkdir -p "$ocr_root/packages"
cd "$ocr_root/packages"
apt-get download tesseract-ocr tesseract-ocr-eng tesseract-ocr-osd libtesseract5 liblept5 libarchive13 libgomp1 libwebp7 libgif7 libopenjp2-7 libtiff6 libjpeg62-turbo libpng16-16 libjbig0 libzstd1 liblzma5 libdeflate0 liblerc4 libwebpmux3
for package in ./*.deb; do dpkg-deb -x "$package" "$ocr_root"; done
...
# Fail the build instead of deploying an OCR binary with missing dependencies/data.
